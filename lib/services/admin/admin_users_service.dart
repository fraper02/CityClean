...
class AdminUsersService {
  ...
  Future<List<UserActivity>> getUserActivity(String userId) async {
    try {
      ...
      for (var item in (results[4] as List)) { if (item['data_operazione'] != null) activities.add(UserActivity(id: item['id'].toString(), type: ActivityType.admin_adjustment, date: DateTime.parse(item['data_operazione']), description: 'Admin: ${item['descrizione'] ?? 'Aggiustamento manuale'}', points: item['punti_assegnati']));}
